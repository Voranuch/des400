...
const config = {
  channelAccessToken: 'P+E8eZRcSNbBoEPGYjBmJExtFjWT3j4o5rF1/7SbnorFSc60fJzKYl9IKmn96qgQ7ry0PFWlaqkZl5O7Zoc9/VGIlC5BcCik0ITxv3SJFuMHSWbk6m+CUwKMPFaJgvTMUrS4s/Ijt7M/OWELImmSawdB04t89/1O/w1cDnyilFU=',
  ...
  for (let event of events) {
    ...
    if (message === 'Check-in') {
      ...
      if (!userSession[userId]) {
        userSession[userId] = { step: 'regist' }; // Start the registration session
        await client.replyMessage(event.replyToken, {
          type: 'text',
        ...
        await client.replyMessage(event.replyToken, {
          type: 'text',
          text: 'You are already in the registration process.',
        });
      }
    } else if (userSession[userId]) {
      // Continue registration process
      if (userSession[userId].step === 'regist') {
        userSession[userId].name = message; // Save the name
